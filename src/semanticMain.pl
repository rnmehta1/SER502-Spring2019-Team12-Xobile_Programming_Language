...
eval_next_cmd(assign(I,V), Env, OpEnv) :-
    is_identifier(I),
    eval_bool_expr(V,R, Env, OpEnv),
    update(I, Env, R, OpEnv).


eval_next_cmd(while(X,Y), Env, OpEnv):-
    eval_bool_expr(X, false, Env,OpEnv).
eval_next_cmd(while(X,Y), Env, OpEnv):-
    write(Env),
    eval_bool_expr(X, true, Env,OpEnv),
    eval_command(Y,Env,OpEnv),
    eval_next_cmd(while(X,Y), Env, OpEnv).
