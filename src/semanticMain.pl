...
/* Looks up the variable and values from existing environment */

lookup(Var, [[_Type, Var, Val]| _], Val).
lookup(Var, [_ | T], Val):- lookup(Var, T, Val).


/* updates the value of an expression */
...
/* predicate to evaluate program environment */

eval_program(start(X), Op):-
    eval_block(X, [], Op),
    lookup(_Z, Op, _New).

/* predicate to evaluate a block of statements */
...
/* predicates to evaluate the declaration block */
...
eval_next_cmd(print(X), Env, OpEnv) :-
    eval_print(X, Env, Val).

eval_print(printExpr(X), Env, Val):- eval_printExpr(X, Env, Val).

eval_printExpr(just_term(X), Env, Val):- eval_nextExpression(X,Env, Val1), 
                                   		 Val = Val1, write(Val).

/* predicate to evaluate arithmetic operations
